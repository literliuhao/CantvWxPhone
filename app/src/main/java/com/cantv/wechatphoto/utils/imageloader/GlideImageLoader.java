...
public class GlideImageLoader implements BaseImageLoader {
    ...
    public void loadImgByRound(Context context, ImageInfo img) {
        Glide.with(context).load(img.getUrl()).dontAnimate().skipMemoryCache(img.isSkipMemoryCache).diskCacheStrategy(img.getDiskCacheStrategy()).error(img.getErrorHolder()).transform(new CenterCrop(context), new GlideRoundTransform(context, img.getCornerSizeDp())).placeholder(img.getPlaceHolder()).priority(img.getPriority()).into(img.getImgView());
    }

    public void loadImgByFitCenter(Context context, ImageInfo img) {
        Glide.with(context).load(img.getUrl()).error(img.getErrorHolder()).placeholder(img.getPlaceHolder()).into(img.getImgView());
    }

    public void loadImgByThumbnail(Context context, ImageInfo img) {
        Glide.with(context).load(img.getUrl()).dontAnimate().skipMemoryCache(img.isSkipMemoryCache).diskCacheStrategy(img.getDiskCacheStrategy()).override(img.getWidth(), img.getHeight()).error(img.getErrorHolder()).placeholder(img.getPlaceHolder()).priority(img.getPriority()).thumbnail(img.getThumbnail()).into(img.getImgView());
    }
