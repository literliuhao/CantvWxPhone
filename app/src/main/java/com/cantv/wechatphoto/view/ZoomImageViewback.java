 ...
 * @author liuhao
 */
public class ZoomImageViewback extends ImageView
		implements OnScaleGestureListener, OnTouchListener, ViewTreeObserver.OnGlobalLayoutListener

{
	private final String TAG = ZoomImageViewback.class.getSimpleName();
		...
		mGestureDetector = new GestureDetector(context, new SimpleOnGestureListener() {
			...
			public boolean onDoubleTap(MotionEvent e) {
				...
				if (getScale() < SCALE_MID) {
					ZoomImageViewback.this.postDelayed(new AutoScaleRunnable(SCALE_MID, x, y), 16);
					isAutoScale = true;
				} else if (getScale() >= SCALE_MID && getScale() < SCALE_MAX) {
					ZoomImageViewback.this.postDelayed(new AutoScaleRunnable(SCALE_MAX, x, y), 16);
					isAutoScale = true;
				} else {
					ZoomImageViewback.this.postDelayed(new AutoScaleRunnable(initScale, x, y), 16);
					isAutoScale = true;
				}
				return true;
			}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (rotate_enable) { // 正在旋转的时候就不接收旋转命令,执行完本次旋转才能旋转
			if (keyCode == 19) {
				if (imagePosition != 0) {
					ZoomImageViewback.this.postDelayed(new RotateRunnable(rotateAngle, angle), 20);
				}
			} else if (keyCode == 20) {
				if (imagePosition != 0) {
					ZoomImageViewback.this.postDelayed(new RotateRunnable(-rotateAngle, -angle), 20);
	...
	private class AutoScaleRunnable implements Runnable {
		...
		public void run() {
			...
			if (((tmpScale > 1f) && (currentScale < mTargetScale))
				...
				ZoomImageViewback.this.postDelayed(this, 16);
			} else {
				// 设置为目标的缩放比例
	...
	private float calculation(float imgWidth, float imgHeight, float width, float height) {
		if (imgWidth > imgWidth) {
			return 1 - (height / imgWidth);
		} else {
			return 1 - (width / imgHeight);
		}
	}
	...
	private class RotateRunnable implements Runnable {
		...
		public RotateRunnable(float degree, float currentDegree) {
			...
			rotate_enable = false;
			ZoomImageViewback.this.setScaleType(ImageView.ScaleType.MATRIX);
		}
		...
		public void run() {
			...
			if (mDegreeCount < mDegree) {
				ZoomImageViewback.this.postDelayed(this, 20);
			} else {
				okDegree = (int) (mDegreeCount % 360);
				rotate_enable = true;
			}
		}
	}
	 ...
	 * @param imgPath
	 */
	private void onReSaveImg(String imgPath) {
		if (okDegree == 0) { // 角度是0就不用重新保存
			return;
		}

		Bitmap bitmap = rotatePicture(imgPath, okDegree);
