...
public class PushAliProgramMsg implements IPushMessage {
	...
	private AliVideo video;// 进播放器播放的单集影片

	@Override
	public PushAliProgramMsg parse(JsonObject data)
		...
		try {
			...
			video = new Gson().fromJson(videoJs, AliVideo.class);
		} catch (Exception e) {
			video = null;
		}
		return this;
	...
	public AliVideo getVideo() {
		return video;
	}

	public void setVideo(AliVideo video) {
		this.video = video;
	}

	@Override
	public String toString() {
		return "PushAliProgramMsg [programId=" + programId + ", chargeType=" + chargeType + ", playIndex=" + playIndex
